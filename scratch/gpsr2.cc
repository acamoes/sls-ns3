  ...
  if (ttl > 1)
    {
      if (!first_packetRcvd[nodeid]) // All nodes generating at the same time
        {
          os_rlf <<  Simulator::Now () << "|" << nodeid << "|" << packet_id << "|" << (int) ttl << "|" << (int) hops << "|" << src << std::endl;
...
GpsrExample::GpsrExample () :
  size (40),
  seed (12345),
  iClient(3), // The node in which will be setup a client application
  iServer(9),// The node in which will be setup a server application
  totalTime (120),
  pcap (true),
  printRoutes (true),
  pkt_size (1024), //initially 512
  data_rate (8192), // initially, 8192 bps, then 40960
  comm_mode ("v2i"),
...
GpsrExample::Configure (int argc, char **argv)
{
  ...
  cmd.AddValue ("seed", "Seed for PseudoRandom Generator.", seed);
  cmd.AddValue ("client", "Client", iClient);
  cmd.AddValue ("server", "Server", iServer);
...
GpsrExample::Run ()
{
	  ...
	  txlogFile = "/home/andre/workspace4/ns3-sls/logs/gpsr-tx_packets_" + runID_str + ".log";
	  rxlogFile = "/home/andre/workspace4/ns3-sls/logs/gpsr-rx_packets_" + runID_str + ".log";
	  headerslogfile = "/home/andre/workspace4/ns3-sls/logs/gpsr-headers_" + runID_str + ".log";
	  ...
	  AnimationInterface anim ("/home/andre/workspace4/ns3-sls/logs/loggpsr.xml");
...
GpsrExample::CreateNodes ()
{
	 ...
	 * **********************************************************************************/
	  ...
	  std::string traceFile = "/home/andre/workspace4/ns3-sls/teste1.tcl";
	  //std::string traceFile = "/home/andre/workspace4/ns3-sls/low-3.1-out.tcl";
	  ...
	  if (comm_mode == "v2v") // Watch out for obu_devices and crashed_device (0)
	  ...
	  else
	    {
	      ...
	      for (uint32_t i = 0; i < size; ++i) {
	        // uint32_t node = nodes.Get (i)->GetId ();
	        Ptr<Node> node = nodes.Get (i);
	        //Apaguei mas aqui a cena e 1 2 e 3 para as tres rsu
